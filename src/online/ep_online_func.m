...
Qf_t.omega.success_probability = zeros(N, 1); % mu_mean; % Success probability for Bernoulli

Qf = repmat(Qf_t, 1, T);

Qg_t = struct; % factor
Qg_t.beta = struct; % Gaussian
Qg_t.beta.natur_mean = zeros(N, 1);%1/sigma2 * X' * y; % Natural mean for Gaussian
%Qg_t.beta.natur_var = diag(1/sigma2 * (X' * X)); %1/sigma2 * (X' * X); % Natural variance for Gaussian
...
Qh_t.omega.success_probability = zeros(N, 1); % mu_mean; % Success probability for Bernoulli
...
Qu_t.mu_previous.mean = zeros(N, 1); % mu_natur_mean;
Qu_t.mu = struct;
Qu_t.mu.mean = zeros(N, 1); % mu_natur_mean;


Qu = repmat(Qu_t, 1, T);
Qu_joint = struct;
Qu_joint.mu.common_var = mu_var;

Qu(1).mu_previous.mean = NaN;
Qu(1).mu.mean = mu_mean;


Q_t= struct; % joint distribution
Q_t.beta = struct; % Gaussian
Q_t.gamma = struct; % Gaussian
...
while((~converged) && (iter_num < max_iter_num))
    iter_num = iter_num + 1;
    %disp(['time: ', datestr(datetime('now')), ', iteration', num2str(iter_num)]);
    %tic;
    ...
    %r:
    [Qr, Qr_joint] = update_r_with_u1(Qh, Qh_joint, Qr, Qr_joint, Qu, Qu_joint, ...
        Sigma, min_var, max_var, eta, run_checks);
    
    %u:
    [Qu, Qu_joint] = update_u_with_u1(Qu, Qu_joint, Qr, Qr_joint, W, min_var, max_var, eta, jitter, run_checks);
    %time(iter_num) = toc;
